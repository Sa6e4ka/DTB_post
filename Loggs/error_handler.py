        ...
        try:
            ...
            if message_or_call:
                ...
                if isinstance(message_or_call, Message):
                    await message_or_call.answer(error_message, reply_markup=button(["Перезапустить бота"]))
                elif isinstance(message_or_call, CallbackQuery):
                    await message_or_call.message.edit_text(error_message, reply_markup=button(["Перезапустить бота"]))
    return wrapper
